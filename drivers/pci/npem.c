 ...
 */
struct npem_led {
	const struct npem_indication *indication;
	struct npem *npem;
...
#define for_each_npem_indication(indicator)\
	for (indicator = indications;\
	     indicator < indications + ARRAY_SIZE(indications); indicator++)

/* Reserved bits are outside specification, count only defined bits. */
static int npem_get_supported_indications_count(u32 capabilities)
{
	const struct npem_indication *indication;
	int cnt = 0;

	for_each_npem_indication(indication)
		if (capabilities & indication->bit)
			cnt++;
...
static enum led_brightness npem_get(struct led_classdev *led)
{
	...
	u32 mask = nled->indication->bit | PCI_NPEM_CTRL_ENABLE;
	...
	/* It is ON if indication bit and PCI_NPEM_CTRL_ENABLED are set */
...
int npem_set(struct led_classdev *led, enum led_brightness brightness)
{
	...
	if (brightness == LED_OFF)
		indications = npem->control & ~(nled->indication->bit);
	else
		indications = npem->control | nled->indication->bit;

	ret = npem_set_active_indications(npem, indications);
	if (ret == 0)
		/*
		 * All writes to this register, including writes that do not
		 * change the register value, are NPEM commands and should
		 * eventually result in a command completion indication in the
		 * NPEM Status Register.
		 *
		 * PCIe Base Specification r6.1 sec 7.9.19.3
		 *
		 * Read NPEM Control register after write to keep cache
		 * in-sync. Controller may also modify active bits, e.g. some
		 * patterns could be mutally exclusive.
		 */
...
static int npem_init(struct pci_dev *dev, int pos, u32 capabilities)
{
	int led_cnt = npem_get_supported_indications_count(capabilities);
	const struct npem_indication *indication;
	struct npem_led *nled;
	struct npem *npem;
	int led_idx = 0;
	...
	for_each_npem_indication(indication) {
		if ((npem->capabilities & indication->bit) == 0)
